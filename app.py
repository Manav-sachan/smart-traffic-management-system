...
# Dictionary to hold the current source for each feed
# feed1 defaults to webcam (0), feed2 defaults to None (waiting for upload)
active_sources = {
    'feed1': 0, 
    'feed2': None 
}
...
def generate_frames(feed_id):
    ...
    # If no source is selected yet, show a standby screen
    ...
    while True:
        ...
        # --- VIRTUAL TRAFFIC LIGHT SIMULATION ---
        ...
        if has_ambulance:
            ...
            cv2.circle(frame, (400, 50), 30, (0, 255, 0), -1) # Draw GREEN Light
            ...
            cv2.circle(frame, (400, 50), 30, (0, 0, 255), -1) # Draw RED Light
...
def upload_video(feed_id):
    ...
    if file.filename != '':
        ...
        file.save(filepath)
        active_sources[feed_id] = filepath # Update source to the uploaded video
    return redirect(url_for('index'))

@app.route('/set_live/<feed_id>/<int:cam_index>')
def set_live(feed_id, cam_index):
    active_sources[feed_id] = cam_index # Update source to webcam (0 or 1)
    return redirect(url_for('index'))
